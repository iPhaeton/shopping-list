...
the single-column `lists` grant — both of which read as tidy-able — are still there, and that no
migration since realtime (deletion, purge schedule, item rename) creates a policy of its own.
