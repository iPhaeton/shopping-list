...
- `colors` — `background`, `surface`, `border`, `text`, `textMuted`, `accent`, `accentDisabled`, `onAccent`
...
inlining the value. The `verify:` command greps `src/` for hex literals outside `theme.ts` and
