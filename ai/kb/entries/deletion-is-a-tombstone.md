...
> live row otherwise, so the mistake is invisible in any test that does not delete something first: a
> list quietly says "2 of 47 done" with 42 in the bin, and "Nothing on this list" stops appearing once
> a list has ever held anything.

Both helpers return a fresh array, so a screen handing one to a `FlatList` memoises it. `countDone`
already filters through `liveItems`; `ShowDeletedToggle` renders **only when the bin is non-empty**,
since a toggle that reveals nothing is noise.
