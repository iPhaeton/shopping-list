...
`SignInScreen` and `SetNameScreen` both follow the convention even though neither navigates (they
take `_props` and ignore it): which stack is mounted is decided by `RootNavigator` from the session
state, so signing in — or, since step 17, setting a name — unmounts the screen rather than navigating
away from it. Not every screen in `RootStackParamList` is mounted at once: `SignIn` exists only while
signed out, `SetName` only while `AuthState` is `nameRequired`.

The `verify:` command sweeps **all** of `src/screens/` — six screens now, `SetNameScreen` since step
